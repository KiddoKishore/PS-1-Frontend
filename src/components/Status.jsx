...
import { statuscode } from '../slices/chargeSlice';
import { IoIosArrowDown } from "react-icons/io";

const Status = () => {
  const [isOpen, setIsOpen] = useState(false); // State to track dropdown toggle
  const [checkedItems, setCheckedItems] = useState({ '200' : true, '422' : true});
  const { statusCode } = useSelector((state) => state.inputs)
...
const handleCheckboxChange = (event) => {
  ...
  // Log checked data after checkbox change
  const checkedData = Object.keys(checkedItems).filter((key) => checkedItems[key]);
  ...
  useEffect(() => {
    // Log checked data after state update
    dispatch(statuscode(Object.keys(checkedItems).filter((key) => checkedItems[key])));
  }, [checkedItems]);
  ...
  return (
    <div className="w-96 m-auto p-5 grid">
      <button
        ...
        type="button"
      >
       <span>Select StatusCode</span> <IoIosArrowDown size={20} />
      </button>
            ...
            <li key={index}>
              <div className="flex items-center">
                <input
                  ...
                  type="checkbox"
                  value={apiItem}
                  checked={checkedItems[apiItem]} // Use checkedItems state if available, otherwise default to false
